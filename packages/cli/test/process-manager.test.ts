  ...
  test('codex bootstrap prompt does not instruct skill loading', () => {
    const tmux = {
      newSession: (_name: string, _commandArgs: string[], env?: Record<string, string>) => {
        ...
        expect(env?.MEET_AI_CODEX_BOOTSTRAP_PROMPT).toContain('Before implementing, produce an execution-grade plan using update_plan.')
        ...
        expect(env?.MEET_AI_CODEX_BOOTSTRAP_PROMPT).toContain('If clarification is needed before planning, use request_user_input.')
