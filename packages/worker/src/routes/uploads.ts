...
uploadsRoute.post('/:id/upload', requireAuth, rateLimitByKey(10, 60_000), async (c) => {
  ...
  return c.json({ id, filename: file.name, size: file.size, content_type: file.type || 'application/octet-stream' }, 201)
})

// GET /api/attachments/:id — download a file
uploadsRoute.get('/attachments/:id', requireAuth, async (c) => {
  const keyId = c.get('keyId')
...
uploadsRoute.get('/:id/messages/:messageId/attachments', requireAuth, async (c) => {
  const keyId = c.get('keyId')
  const roomId = c.req.param('id')
...
uploadsRoute.patch('/attachments/:id', requireAuth, async (c) => {
  const keyId = c.get('keyId')
  const attachmentId = c.req.param('id')
  const db = queries(c.env.DB)
