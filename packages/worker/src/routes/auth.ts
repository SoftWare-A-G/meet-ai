...
export const authRoute = new Hono<AppEnv>()
  ...
  .post('/share', requireAuth, async c => {
    ...
    await c.env.DB.prepare(
      'INSERT INTO share_tokens (token, key_id, api_key, expires_at) VALUES (?, ?, ?, ?)'
    )
      .bind(token, keyId, rawKey, expiresAt)
      .run()
  ...
  .get('/claim/:token', async c => {
    ...
    const row = await c.env.DB.prepare(
      'SELECT token, api_key, expires_at, used FROM share_tokens WHERE token = ?'
    )
      .bind(token)
      .first<{ token: string; api_key: string; expires_at: string; used: number }>()
    ...
    // Mark as used
    await c.env.DB.prepare('UPDATE share_tokens SET used = 1 WHERE token = ?').bind(token).run()

    return c.json({ api_key: row.api_key })
  })
