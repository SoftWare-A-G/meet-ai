...
import { THEME_PRESETS, DEFAULT_SCHEMA, FONT_SCALE_OPTIONS, DEFAULT_FONT_SCALE } from '../../lib/constants'
...
type SettingsModalProps = {
  ...
  onSave: (schema: string, fontScale: string) => void
  onClose: () => void
}

export default function SettingsModal({ currentSchema, currentFontScale, onSave, onClose }: SettingsModalProps) {
  ...
  const savedFontScale = currentFontScale
  ...
  const handleFontScaleChange = useCallback((val: string | null) => {
    const v = val ?? DEFAULT_FONT_SCALE
    setFontScale(v)
    applyFontScale(v)
  }, [])

  const handleSave = useCallback(() => {
    onSave(schema.trim(), fontScale)
  }, [schema, fontScale, onSave])

  const handleCancel = useCallback(() => {
    applySchema(savedSchema)
    applyFontScale(savedFontScale)
    onClose()
  }, [savedSchema, savedFontScale, onClose])
  ...
  return (
    <Dialog open onOpenChange={handleOpenChange}>
      <DialogContent className="bg-chat-bg text-msg-text max-h-[80vh] w-[460px] max-w-[90vw] sm:max-w-[460px] grid-cols-1 gap-0 overflow-y-auto p-6" showCloseButton={false}>
        ...
        <label className="mb-1 block text-[13px] font-semibold">Font Size</label>
        <Select value={fontScale} onValueChange={handleFontScaleChange}>
          <SelectTrigger className="mb-1 w-full">
            <SelectValue>
              {FONT_SCALE_OPTIONS.find(o => o.value === fontScale)?.label ?? 'Default'}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        <p className="text-muted-foreground mb-2 text-xs">
          Scales text across the entire interface. Useful for large or high-resolution displays.
        </p>
