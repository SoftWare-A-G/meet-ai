...
export default function FormattingToolbar({ textareaRef, onSend, onAttach }: FormattingToolbarProps) {
  ...
  return (
    <div className="flex items-center gap-1 px-2 py-1 border-t border-border">
      {onAttach && (
        <button className="bg-transparent border-none text-msg-text opacity-45 cursor-pointer p-1 rounded flex items-center justify-center hover:opacity-80 hover:bg-white/10" title="Attach file" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={onAttach} onClick={onAttach}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" /></svg>
        </button>
      )}
      <button className="bg-transparent border-none text-msg-text opacity-45 cursor-pointer p-1 rounded flex items-center justify-center hover:opacity-80 hover:bg-white/10" title="Bold" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={() => handleFormat('bold')} onClick={() => handleFormat('bold')}>
      ...
      <button className="bg-transparent border-none text-msg-text opacity-45 cursor-pointer p-1 rounded flex items-center justify-center hover:opacity-80 hover:bg-white/10" title="Italic" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={() => handleFormat('italic')} onClick={() => handleFormat('italic')}>
      ...
      <button className="bg-transparent border-none text-msg-text opacity-45 cursor-pointer p-1 rounded flex items-center justify-center hover:opacity-80 hover:bg-white/10" title="Code" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={() => handleFormat('code')} onClick={() => handleFormat('code')}>
      ...
      <button className="bg-transparent border-none text-msg-text opacity-45 cursor-pointer p-1 rounded flex items-center justify-center hover:opacity-80 hover:bg-white/10" title="Link" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={() => handleFormat('link')} onClick={() => handleFormat('link')}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" /></svg>
      </button>
      <div className="flex-1" />
      <button className="p-1 border-none rounded w-7 h-7 bg-active text-active-text cursor-pointer flex items-center justify-center shrink-0 hover:brightness-110" title="Send" onMouseDown={preventBlur} onTouchStart={preventBlur} onTouchEnd={onSend} onClick={onSend}>
