import { useEffect, useRef, useCallback } from 'hono/jsx/dom'
...
export default function ResultState({ apiKey }: { apiKey: string }) {
  ...
  return (
    <div class="flex flex-col gap-5 text-left">
      <div class="flex items-center gap-2 text-sm text-text-secondary stagger-in">
        <span class="size-2 rounded-full bg-green-500 shrink-0" /> Your key is ready
      </div>

      <div class="shimmer flex gap-2 items-center relative max-[520px]:flex-col stagger-in">
        <input
          type="text"
          value={apiKey}
          readonly
          class="flex-1 py-3 px-3.5 border border-edge-light border-l-[3px] border-l-blue-600 rounded-lg bg-edge text-text-primary font-mono text-sm outline-none transition-[border-color] duration-150 focus:border-edge-hover focus:border-l-blue-600 max-[520px]:w-full"
        />
        <button
      ...
      <p class="text-sm text-text-secondary mt-2 stagger-in">
      ...
      <div class="stagger-in">
        <h3 class="text-sm font-semibold mb-5 text-text-primary">Quick Start</h3>
        <QuickStartSteps />
      </div>
      ...
      <p class="actions-helper text-sm text-text-muted text-center stagger-in">
