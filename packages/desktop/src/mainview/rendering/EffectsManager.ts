...
import type { FrameContext, SpawnBeam, ParticleEffect, FloatingText, ZoneParticleSystem } from '../types'
...
export class EffectsManager {
	...
	createSpawnBeam(x: number, z: number, elapsed: number): void {
		...
		mesh.scale.y = 0.01
		this.scene.add(mesh)
		this.spawnBeams.push({ mesh, startTime: elapsed, duration: 1.0 })
	}

	createParticleBurst(x: number, y: number, z: number, color: number, count: number, elapsed: number): void {
	...
	createFloatingText(x: number, y: number, z: number, text: string, color: string, elapsed: number): void {
