	...
	execute(data: { team_name: string; members: TeamMember[] }): void {
		...
		for (const member of data.members) {
			...
			if (existing) {
				...
				scene.updateAgentLabel(member.teammate_id, this.actions.getAgentLabelText(existing.name, existing.role, existing.level, existing.id))
				...
				const agentData = createAgentData(member.teammate_id, member.name, member.color, member.role, member.status, spawnIndex)
		...
		if (idleAgents.length > 1) {
			idleAgents.forEach((agent, i) => {
				...
				scene.setAgentTargetPosition(agent.id, Math.cos(angle) * SPAWN_RADIUS, Math.sin(angle) * SPAWN_RADIUS)
			})
		}
