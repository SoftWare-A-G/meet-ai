...
import { BOB_AMPLITUDE, GLOW_PULSE_SPEED, IDLE_SPIN_SPEED, ZONES, BUBBLE_DURATION } from '../constants'
...
export class ThreeSceneAdapter implements ISceneAdapter {
	...
	setAgentGlow(id: string, status: AgentData['status']): void {
		...
		switch (status) {
			case 'working': mat.color.setHex(0x22cc55); break
			case 'talking': mat.color.setHex(0xeebb33); break
			case 'error':   mat.color.setHex(0xee3333); break
			default:        mat.color.setHex(0x3388ff)
		}
	}
	...
	update(frame: FrameContext, agents: Iterable<AgentData>): void {
		...
		for (const agent of agents) {
			...
			const lerped = lerpPosition(rs.position.x, rs.position.z, rs.targetPosition.x, rs.targetPosition.z, delta)
			...
			if (agent.status === 'idle') {
				...
				const targetAngle = Math.atan2(rs.targetPosition.x - rs.position.x, rs.targetPosition.z - rs.position.z)
